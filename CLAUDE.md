...
2. **Direct GATT connection (trains)** — `controllers/train_controller.py` connects directly to each train hub via `BleakClient` (GATT service `00001623-...`, characteristic `00001624-...`), the same protocol `servers/lego_service.py` (a separate, standalone interactive CLI tool, not used by the FastAPI web service) has always used. Train hubs run **stock LEGO Powered Up firmware**, not Pybricks. `TrainController` connects directly by BLE address (from `Settings.train_hub_mapping`, wired in via `LegoController.__init__`) rather than discovering hubs with a BLE scan — a scan would open its own BlueZ discovery session on the same adapter/D-Bus connection `SwitchController`'s continuous scan already holds, and BlueZ rejects a second concurrent `StartDiscovery` from the same client with `[org.bluez.Error.InProgress]`, permanently (not just a transient race) since the switch scan never stops. Connections are persistent `BleakClient`s per BLE address (auto-reconnecting on disconnect), and motor commands are `write_gatt_char` calls encoding a LEGO Wireless Protocol 3.0 payload. Trains are identified throughout by their **BLE address** (e.g. `"90:84:2B:18:28:36"`), not an integer hub ID — this differs from switches, which are still addressed by integer broadcast-channel hub ID.
