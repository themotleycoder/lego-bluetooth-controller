...
The dispatcher adds autonomous, block-protected train movement on top of the manual BLE control path above — RFID tags at track block boundaries let the system know where each train is, instead of relying purely on manual `/train` calls.
...
- `dispatcher/track_model.py::TrackModel` models the track as a directed graph of RFID tag nodes and `TrackEdge`s (block segments), each optionally requiring specific switch positions; `TrackModel.tag_id_for_uid()` maps a physical scanned UID to a logical tag_id. `build_sample_topology()` ships a 14-tag/7-switch inner+outer-loop sample.
- `dispatcher/block_manager.py::BlockManager` grants/queues block entry per edge and sets required switches (all-or-nothing) via the *same* `SwitchController.send_command_with_retry` used by the REST API. Contention between two trains wanting the same block is resolved by route-hop distance (farther train holds; ties break lexically by train_id — see `resolve_contention`'s docstring for the full rationale).
- `dispatcher/dispatcher.py::Dispatcher` is the orchestrator: on each tag event it updates position, releases the block just left, and either advances the train (stop/resume via the *same* `TrainController.handle_command` used by the REST API) or holds it. A watchdog stops every train if one misses an expected tag within `dispatcher_watchdog_timeout`, auto-clearing once that train's tag reappears.
...
- Manual REST commands (`/train`, `/selfdrive`) **bypass the dispatcher entirely** in this version — no reconciliation between manual control and dispatcher-driven movement.
